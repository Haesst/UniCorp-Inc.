...
PowerupMoveSpeed::PowerupMoveSpeed() {}

PowerupMoveSpeed::~PowerupMoveSpeed() {}

void PowerupMoveSpeed::onCollision(Tag tagau)
{
	switch (tagau)
	{
	case Tag::Playerau:
...
void PowerupMoveSpeed::PowerupEffect()
{
	...
	player->currentPowerup = "MoveSpeed";
...
PowerupMoveSpeed::PowerupMoveSpeed(FG::SpriteManager* spriteManagerRef, FG::Vector2D pos)
{
	position.x = pos.x;
	position.y = pos.y;
	...
	sprite = spriteManager->CreateSprite("../TestingAssets/PowerUpPlayerSpeed.png", 1, 1, 32, 32);
	rect = { 0,0,32,32 };
...
void PowerupMoveSpeed::Update(float deltaTime)
{
...
void PowerupMoveSpeed::Render(FG::Camera* const camera)
{
